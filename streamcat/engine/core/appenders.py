...
class VisDataDestAppender():
    ...
    def do_append(self, flow_cmd:FlowCommand, point:Point, vis_args:dict):
        ...
        if 'streamz' in point.src_tubes[0].port.types:
            # 接続先Pointの入力Portの型が'streamz'の場合は、Steramz用の前処理Stepを繋げる
            return self._append_streamz(flow_cmd, point, vcmd_args, vcmd_is_table)
        else:
            # 'mcmd'の場合は、nysol_python用の前処理Stepを繋げる
            return self._append_mcmds(flow_cmd, point, vcmd_args, vcmd_is_table)

    def _append_streamz(self, flow_cmd:FlowCommand, point:Point, vcmd_args:dict, vcmd_is_table:bool):   
        # ToList Stepを作成する 
        tolist_cmd = CommandLink('streamz_tolist').resolve()
        tolist_step = Step(tolist_cmd.label, tolist_cmd, vcmd_args)

        # ToListの出力Pointを作成する
        point_id = point.id + '_tolist_o'
        tolist_point_o = Point(point_id, Tube(Port('o', 'streamz'), tolist_step))

        # pointにToList Stepを繋げる
        point.add_dst_tube(Tube(Port('i', 'streamz'), tolist_step))
...
class RunsCommandAppender():
    ...
    def __init__(self):
        # Streamz
        self._streamz_runs_cmd_appender = RunsCommandAppender.Appender('streamz_run', 'streamz')
        # nysol_python
        self._mcmd_runs_cmd_appender = RunsCommandAppender.Appender('runs', 'mcmd')

    def do_append(self, flow_cmd:FlowCommand, point1:Point, point2:Point=None):
        if 'streamz' in point1.src_tubes[0].port.types:
            # 接続先Pointの入力Portの型が'streamz'の場合は、StreamzのRunStepを繋げる
            return self._streamz_runs_cmd_appender.do_append(flow_cmd, point1, point2)
        else:
