...
class VisDataDestAppender():
    ...
    def do_append(self, flow_cmd:FlowCommand, point:Point, vis_args:dict):
        ...
        if 'mysol' in point.src_tubes[0].port.types:
            # 接続先Pointの入力Portの型が'mysol'の場合は、MYSOL用の前処理Stepを繋げる
            return self._append_mysol(flow_cmd, point, vcmd_args, vcmd_is_table)
        else:
            # 'mcmd'の場合は、nysol_python用の前処理Stepを繋げる
            return self._append_mcmds(flow_cmd, point, vcmd_args, vcmd_is_table)

    def _append_mysol(self, flow_cmd:FlowCommand, point:Point, vcmd_args:dict, vcmd_is_table:bool):   
        # ToList Stepを作成する 
        tolist_cmd = CommandLink('outl').resolve()
...
class RunsCommandAppender():
    ...
    def __init__(self):
        # MYSOL
        self._mysol_runs_cmd_appender = RunsCommandAppender.Appender('mysol_runs', 'mysol')
        # nysol_python
        self._mcmd_runs_cmd_appender = RunsCommandAppender.Appender('runs', 'mcmd')

    def do_append(self, flow_cmd:FlowCommand, point1:Point, point2:Point=None):
        if 'mysol' in point1.src_tubes[0].port.types:
            # 接続先Pointの入力Portの型が'mysol'の場合は、MYSOLのRunStepを繋げる
            return self._mysol_runs_cmd_appender.do_append(flow_cmd, point1, point2)
        else:
